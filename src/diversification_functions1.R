...
fitCRBD <- function(phy, nopt=5, lmin=0.001, lmax=5.0, MAXBAD = 200, condition = T){
	...
	for (i in 1:nopt){
		...
		resx <- try(optim(c(lam, mu) ,fx, method='L-BFGS-B', control=list(maxit=1000, fnscale=-1), lower=lmin, upper=lmax, condition.surv = condition), silent=T)
		while (class(resx) == 'try-error'){
